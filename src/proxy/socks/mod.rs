use std::{net::{IpAddr, SocketAddr}, ops::Add, str::FromStr};
...
use crate::proxy::{Address, AnyStreamTrait, ConnectionSession, DomainSession, ProxyStream, TcpInbound};
...
// as server
async fn handshake_as_server<T>(stream: &mut T) -> Result<ConnectionSession>
where
    T: AnyStreamTrait
{
    let mut buf = vec![0; 3];
