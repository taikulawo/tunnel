...
use app::{Dispatcher, DnsClient, InboundManager, OutboundManager, Router, UdpAssociationManager};
...
pub fn start(
    ...
    shutdown_handler: BoxFuture<'static, ()>,
) -> anyhow::Result<()> {
    ...
    let nat = UdpAssociationManager::new(dispatcher.clone());
    let inbound_manager = InboundManager::new(config.inbounds.clone(), Arc::new(nat));
