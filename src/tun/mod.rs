use std::{error::Error, io::{self, Cursor, ErrorKind}, net::{IpAddr, Ipv4Addr, Ipv6Addr, SocketAddr, SocketAddrV4}};
use ipnet::Ipv4Net;
use log::{
    error
};
use etherparse::{IpHeader, Ipv4Header, PacketBuilder, PacketHeaders, ReadError, TcpHeader, TransportHeader};
...
impl Tun {
    pub async fn new() -> io::Result<Tun> {
        ...
        let tcp_tun = TcpTun::new(tun_network.into()).await.expect("tcp tun error");
        Ok(Tun {
            device,
            tcp_tun,
        })
    ...
    async fn handle_ip_packet(&self, packet: &mut[u8]) -> io::Result<bool> {
        ...
        match ip_packet.transport {
            Some(TransportHeader::Tcp(ref mut tcp_header)) => {
                ...
                match ip_header {
                    IpHeader::Version4(v4_ip_header) => tcp_header.checksum = tcp_header.calc_checksum_ipv4(&v4_ip_header, ip_packet.payload).expect("tcp calculate check sum error"),
                    IpHeader::Version6(v6_ip_header) => tcp_header.checksum = tcp_header.calc_checksum_ipv6(&v6_ip_header, ip_packet.payload).expect("tcp calculate check sum error"),
                }
                ...
                ip_header.write(&mut cursor).expect("ip header write failed!");
                tcp_header.write(&mut cursor).expect("tcp header write failed!");
            },
